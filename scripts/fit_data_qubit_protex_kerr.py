...
if 1==1:
    ...
    def get_freqs_kerrs_from_vals(x, params):
        print('eval')
        f = np.zeros((3, len(x)))
        K = np.zeros((3, len(x)))
        ...
        for kk, xx in enumerate(x):
            res1, res2, fs, fs_diff, Xi3s, Xi4s, coeff, _Xi22 = c.get_freqs_kerrs(phi_ext_s_0=SL*(xx+SL0), phi_ext_l_0=BL*(xx+BL0))
            sort_indices = np.argsort(fs)
            f[:, kk] = fs[sort_indices]
            K[:, kk] = Xi4s[2:][sort_indices]
        return f, K
    ...
    def get_Kb(x, params):
         f, K = get_freqs_kerrs_from_vals(x, params)
         return K[1]
...
if 1==1: ## LOAD DATA ##
    folder_root= '..\data\qubit_protex'
    folder_data ='analyzed_data\sweep_DC_specVNA_calib_Kerr_follow_in3outB_pump3_Bobine_004'
    folder_data = os.path.join(folder_root, folder_data)

#    buffer_data = np.load(folder_data+filename_buffer)
    buffer_flux_freq = np.load(os.path.join(folder_data, 'DC_fit_freq.npy'))
    buffer_freq = np.load(os.path.join(folder_data, 'freq_on.npy'))
    buffer_flux_kerr = np.load(os.path.join(folder_data, 'DC_fit_kerr.npy'))
    buffer_kerr = np.load(os.path.join(folder_data, 'kerr.npy'))
...
#        ax1.plot(mem_flux, mem_freq/1e9, 'o')
        ax2.plot(buffer_flux_freq, buffer_freq/1e9, 'o')
        ax2.set_xlabel('DC (V)')
...
if 1==1: # FIT BUFFER
    folder_img = os.path.join(folder_root,'figures')
    params = Parameters()
...
#    params = outb.params
    ...
    ax[0].plot(buffer_flux_freq, buffer_freq/1e9,'o', label='data')
    ax[1].plot(buffer_flux_kerr, buffer_kerr/1e6,'o', label='data')
    if iffit:
        ax[0].plot(x, get_fb(x, outb.params)/1e9,linewidth=2.0, label='theory')
        ax[1].plot(xK, 850*get_Kb(xK, outb.params)/1e9,linewidth=2.0, label='theory')
    else:
        ax[0].plot(x, get_fb(x, params)/1e9, label='guess')
        ax[1].plot(xK, 850*get_Kb(xK, params)/1e9, label='guess')
#    ax.plot([min(buffer_flux), max(buffer_flux)], outb.params['wb']/2/np.pi/1e9*np.ones((2)), label = 'wb')
    ...
    ax[1].set_ylabel('Frequency (MHz)', fontsize=fs)
    ax[0].set_title('Buffer mode spectroscopy', fontsize=fs)
    ax[1].set_title('Buffer Kerr', fontsize=fs)
    ax[1].set_ylim(-30,30)
    ax[1].legend(fontsize=fs)
    ax[0].legend(fontsize=fs)
    plt.savefig(folder_img+'spec_kerr_buff.pdf')
