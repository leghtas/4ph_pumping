...
class CircuitPump2Snail(c.Circuit):
    ...
    def __init__(self, wb, Zb, LJ, L, printParams=True):
...
#        self.varying_params={'ECca':0}
        ...
        self.U_str = 'ELc/2/hbar*(p-(pext_1-pext_2)/2)**2 \
                      + ELb/2/hbar*pLb**2 \
                      - (EJ/hbar)*2*cos((pext_1+pext_2)/2)*cos(p)'
#
   
#        self.U_str = 'ELb/2/hbar*pLb**2 \
#                      + ELc/2/hbar*(p1+pext_1)**2 \
#                      - (EJ/hbar)*cos(p1) \
#                      - (EJ/hbar)*cos(pext_1+pext_2+p1)'
# ELb/2/hbar*pLb**2 \
   #                   + 
                 
        self.T_str = '(1/16.)*(hbar/ECb)*(dp + dpLb)**2 \
                     +(1/16.)*(hbar/ECb)*(dp)**2/1000'
